...
ZvecSearch는 **마크다운 파일을 지식의 단일 원천(Single Source of Truth)**으로 다룬다. 마크다운은:

- **사람이 읽고 편집할 수 있다** — 독점 포맷 없음, 종속 없음
- **Git 친화적이다** — 버전 관리, diff, 협업이 바로 가능
- **구조적 의미가 있다** — 헤딩(`#`, `##`, `###`)이 자연스러운 의미 단위 경계를 제공하여 청킹에 활용
- **AI 워크플로우의 기본 포맷이다** — Claude, ChatGPT 등 대부분의 LLM 도구가 마크다운으로 읽고 쓴다

별도 데이터베이스에 데이터를 가져오는 대신, ZvecSearch는 기존 마크다운 파일을 그 자리에서 인덱싱한다. 문서는 원래 위치에 그대로 — ZvecSearch는 검색만 가능하게 한다.
...
**이 프로젝트는 개인 에이전트 개발 용도로 만든 것이며, 아직 매우 초기 상태입니다.** 사용에 주의해 주세요.

## 감사

이 프로젝트의 영감과 기반이 되어준 [zvec](https://github.com/alibaba/zvec), [OpenClaw](https://github.com/openclaw/openclaw), [memsearch](https://github.com/zilliztech/memsearch)에 감사드립니다.
